...
where array pa[0..2], pb[0..2], pmiss[0..2] contains (mass,px,py) 
	...
	3. Use MT2Functor::get_mt2() to obtain the value of mt2:
...
const float MT2Functor::RELATIVE_PRECISION = 0.00001;
const float MT2Functor::ABSOLUTE_PRECISION = 0.0;
const float MT2Functor::ZERO_MASS = 0.0; 
const float MT2Functor::MIN_MASS = 0.1; 
const float MT2Functor::SCANSTEP = 0.1;

MT2Functor::MT2Functor(){
	solved = false;
	momenta_set = false;
	mt2_b  = 0.;
	scale = 1.;
...
double MT2Functor::get_mt2(){
	if (!momenta_set)
	{
		cout << "MT2Functor::get_mt2() ==> Please set momenta first!" << endl;
		return 0;
	}
...
void MT2Functor::set_mn(double mn0){
...
void MT2Functor::print(){
	cout << " MT2Functor::print() ==> pax = " << pax*scale << ";   pay = " << pay*scale << ";   ma = " << ma*scale <<";"<< endl;
	cout << " MT2Functor::print() ==> pbx = " << pbx*scale << ";   pby = " << pby*scale << ";   mb = " << mb*scale <<";"<< endl;
	cout << " MT2Functor::print() ==> pmissx = " << pmissx*scale << ";   pmissy = " << pmissy*scale <<";"<< endl;
	cout << " MT2Functor::print() ==> mn = " << mn_unscale<<";" << endl;
}

//special case, the visible particle is massless
void MT2Functor::mt2_massless(){
...
//look for when both parablos contain origin  
	...
	if (nsols_high == nsols_low)
	{
		...
		if(foundhigh==0) 
		{
			...
			if(verbose > 0) cout << "MT2Functor::mt2_massless() ==> Deltasq_high not found at event " << nevt <<endl;
	...
	if(nsols_high == nsols_low)
	{ 
		if(verbose > 0) cout << "MT2Functor::mt2_massless() ==> error: nsols_low=nsols_high=" << nsols_high << endl;
		if(verbose > 0) cout << "MT2Functor::mt2_massless() ==> Deltasq_high=" << Deltasq_high << endl;
		if(verbose > 0) cout << "MT2Functor::mt2_massless() ==> Deltasq_low= "<< Deltasq_low << endl;
...
//number of solutions at Deltasq_low should not be larger than zero
	...
	{
//cout << "MT2Functor::mt2_bisect() ==> nsolutions(Deltasq_low) > 0"<<endl;
		mt2_b = (double) sqrt(mnsq+Deltasq0);
		return;
	}
...
//if nsols_high=4, also need a scan because we may find the wrong tangent point.
	...
	//foundhigh = scan_high(Deltasq_high);
		...
		if(foundhigh == 0) 
		{
			if(verbose > 0) cout << "MT2Functor::mt2_bisect() ==> Deltasq_high not found at event " << nevt << endl;
...
int MT2Functor::find_high(double & Deltasq_high){
	double x0,y0;
	x0 = (c1*d1-b1*e1)/(b1*b1-a1*c1);
	y0 = (a1*e1-b1*d1)/(b1*b1-a1*c1);
...
//inline
int MT2Functor::signchange_n(long double t1, long double t2, long double t3, long double t4, long double t5){
	int nsc;
	nsc=0;
	if(t1*t2>0) nsc++;
	if(t2*t3>0) nsc++;
...
//inline
int MT2Functor::signchange_p(long double t1, long double t2, long double t3, long double t4, long double t5){
	int nsc;
	nsc=0;
	if(t1*t2<0) nsc++;
	if(t2*t3<0) nsc++;
