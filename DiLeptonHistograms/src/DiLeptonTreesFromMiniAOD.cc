...
DiLeptonTreesFromMiniAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  uncorrectedMet.SetPtEtaPhiE(mets->front().uncorrectedPt(), 0,	mets->front().uncorrectedPhi(), mets->front().uncorrectedPt());
  floatEventProperties["met"] = uncorrectedMet.Pt();
  ...
  floatEventProperties["type1Met"] = metVector.Pt();
  tLorentzVectorEventProperties["vMetType1"] = metVector;
...
DiLeptonTreesFromMiniAOD::makeCombinations ( const std::string &treeName, const std::vector<aT> &a, const std::vector<bT> &b,const std::vector<pat::PackedCandidate>&pfCands, const edm::Event &ev, const pat::MET &patMet, const TLorentzVector &MHT, const std::map<std::string, int> &intEventProperties, const  std::map<std::string, float> &floatEventProperties, const  std::map<std::string, TLorentzVector> &tLorentzVectorEventProperties)
{
  ...
  *(tLorentzVectorBranches_[treeName]["vMet"]) = uncorrectedMet2;
...
int 
DiLeptonTreesFromMiniAOD::getMotherPdgId( const reco::GenParticle &p)
{
  int result = -9999;
  if(p.mother() != NULL){
    if(p.status() == 3)
...
int 
DiLeptonTreesFromMiniAOD::getLeptonPdgId( const reco::GenParticle &p)
{
  int result = -9999;
  if(p.status() == 3)
     result = p.pdgId();
...
const TLorentzVector DiLeptonTreesFromMiniAOD::getMomentum(const  pat::Tau &tau)
{
  const TLorentzVector result = TLorentzVector(tau.px(), tau.py(), tau.pz(), tau.energy());
  return result;
}

float DiLeptonTreesFromMiniAOD::getId(const  pat::Electron &e)
{
  //  if (e.isEE())
...
float DiLeptonTreesFromMiniAOD::getId(const  pat::Muon &mu)
{
...
float DiLeptonTreesFromMiniAOD::getId(const  pat::Tau &tau)
{
...
float DiLeptonTreesFromMiniAOD::getDeltaB(const  pat::Tau &tau)
{
