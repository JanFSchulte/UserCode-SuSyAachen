...
class OnTheFlyBadGlobalMuonTagger : public edm::stream::EDFilter<> {
    public:
        explicit OnTheFlyBadGlobalMuonTagger(const edm::ParameterSet & iConfig);
        virtual ~OnTheFlyBadGlobalMuonTagger() {}
...
OnTheFlyBadGlobalMuonTagger::OnTheFlyBadGlobalMuonTagger(const edm::ParameterSet & iConfig) :
    muons_(consumes<edm::View<reco::Muon>>(iConfig.getParameter<edm::InputTag>("src"))),
...
bool 
OnTheFlyBadGlobalMuonTagger::filter(edm::Event & iEvent, const edm::EventSetup & iSetup) {
    using namespace edm;
...
DEFINE_FWK_MODULE(OnTheFlyBadGlobalMuonTagger);
