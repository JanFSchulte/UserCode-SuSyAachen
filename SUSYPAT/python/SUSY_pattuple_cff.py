...
def loadPF2PAT(process,mcInfo,jetMetCorrections,extMatch,doSusyTopProjection,doType1MetCorrection,doType0MetCorrection,postfix):
    ...
    usePF2PAT(process,runPF2PAT=True,
        ...
        runOnMC             = (mcInfo==1),
        postfix             = postfix,
        jetCorrections      = ('AK4PFchs', jetMetCorrections,"None"),
...
def loadPATTriggers(process,HLTMenu,theJetNames,electronMatches,muonMatches,tauMatches,jetMatches,photonMatches):
    ...
    process.patJetMatchAK4Calo = cleanJetTriggerMatchHLTJet15U.clone(matchedCuts = cms.string( jetMatches ),src='cleanPatJetsAK4Calo')
    switchOnTriggerMatchEmbedding( process, ['patJetMatchAK4Calo'], hltProcess=HLTMenu)
    ...
    pfSwitchOnTriggerMatchEmbedding( process, ['patJetMatchPF'], 'selectedPatJetsPF', 'cleanPatJetsAK4CaloTriggerMatch' )
    #Taus
...
def addTypeIIMet(process) :
    ...
    from PhysicsTools.PatAlgos.recoLayer0.jetMETCorrections_cff import metJESCorAK4CaloJet
    process.metJESCorAK4CaloJetTypeII = metJESCorAK4CaloJet.clone()
    process.metJESCorAK4CaloJetTypeII.useTypeII = True
    process.metJESCorAK4CaloJetTypeII.hasMuonsCorr = False
    # Add muon corrections for above II reco::MET
    process.metJESCorAK4CaloJetMuonsTypeII = process.metJESCorAK4CaloJetMuons.clone(
        uncorMETInputTag = cms.InputTag('metJESCorAK4CaloJetTypeII')
        )
    # Add to recoLayer0 sequence
    process.patMETCorrections.replace(
        process.metJESCorAK4CaloJet,
        (process.metJESCorAK4CaloJetTypeII*
         process.metJESCorAK4CaloJetMuonsTypeII)+
        process.metJESCorAK4CaloJet
        )
    # Add pat::MET with Type II correction
    process.patMETsAK4CaloTypeII = process.patMETs.clone(
        metSource = cms.InputTag("metJESCorAK4CaloJetMuonsTypeII")
        )
    # Add to producersLayer1 sequence
    process.patDefaultSequence.replace(
        process.patMETsAK4Calo,
        process.patMETsAK4Calo+
        process.patMETsAK4CaloTypeII
        )
...
def addTagInfos(process,jetMetCorrections):
    ...
    switchJetCollection( process,
                     ...
                     jetCorrections=('AK4Calo', jetMetCorrections,"None"))
...
def addSUSYJetCollection(process,jetMetCorrections,jets = 'IC5Calo',mcVersion='',doJTA=True,doType1MET=True,doJetID=True,jetIdLabel=None):
    ...
    algorithm = jets[0:3]
    print "*************************************************************"
    print "Badly hacked SUSY_pattuple_cff.py to use AK4PFCHS as input instead of AK4PF"
    print "*************************************************************"
    print jets
    print algorithm
...
def addJetMET(process,theJetNames,jetMetCorrections,mcVersion):
    ...
    #-- Tune contents of jet collections (hacked to use AK4PFCHS instead of AK4PF)  -----------------------------------------
    theJetNames = ['AK4PFCHS','']
    #~ theJetNames.append('')
    for jetName in theJetNames:
    ...
    process.cleanPatJetsAK4Calo = process.cleanPatJets.clone()
    process.patMETsAK4Calo      = process.patMETs.clone()
    ...
    process.patCandidateSummary.candidates.append(cms.InputTag('patMHTsAK4Calo'))
